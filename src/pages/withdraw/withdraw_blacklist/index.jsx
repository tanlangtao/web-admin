...
export default () => {
    ...
    const fetchData = async (page, limit, user_id) => {
        const res = await withdrawBlack("getWithdrawBlack", { page, limit, user_id });
        setTable({ data: res.data.list || [], count: res.data.count || 0, current: page });
        if (res.status === 0) {
    ...
    const initColumns = [
        ...
        },
        {
            title: "状态",
            dataIndex: "status",
            render: (text) => (text === 0 ? "取消封号" : text === 1 ? "封号中" : text),
        },
        // {
        //     title: "操作账号",
    ...
    const onSubmitHandled = async () => {
        const { user_id, reason, buttonStatus, id, status, search_value } = ref.current;
        if (!user_id) {
            message.info("请输入user_id");
            return;
        }
    ...
    const check_history = async (record) => {
        const res = await withdrawBlack("getWithdrawBlackLog", { black_id: record.id });
        if (res.status === 0) {
            message.success(res.msg);
            ...
            {is_modal_show && (
                ...
                >
                    ...
                    <Input.TextArea
                        ...
                        placeholder="请输入备注"
                        defaultValue={ref.current.buttonStatus !== 1 ? ref.current.reason : ""}
                        onChange={(e) => {
