...
import { Card, Table, Modal, Icon, Input, Select, Popover, message, Button, Tooltip } from "antd";
	...
	initColumns = () => [
		{
			...
			render: (text, record, index) => (
				<span>
					<Button type="primary" size="small" onClick={() => this.edit(record, index)}>
						编辑
					</Button>
				</span>
			),
		...
		},
		{
			...
			render: (text, record, index) => {
				...
				if (diffDays < 31 && record.status == 1) {
					return (
						text ||
						(<Button type="default" size="small" onClick={() => this.pick(record, index)}>
							认领
						</Button>)
					)
				}
