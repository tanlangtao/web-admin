...
class ProxySetting extends Component {
		...
		},
		{
			...
			render: (text, record, index) => (
				<span>
					...
					&nbsp;&nbsp;&nbsp;
					<Button type="primary" size="small" onClick={() => this.changePid(record)}>
						转移
					</Button>
				</span>
			),
	...
	changePid = record => {
		Modal.confirm({
			title: "修改上级代理",
			content: <Input onBlur={e => this.setState({ new_proxy_user_id: e.target.value })} />,
			onOk: async () => {
	...
	checkBalance = async record => {
		...
		if (res.status === 0) {
			Modal.success({
				title: "实时余额",
				content: `代理${record.id}实时余额是 : ${res.data ? (Math.floor(res.data.balance * 100) / 100).toFixed(2) : "0.00"
					}`,
			});
		} else {
			message.info(res.msg || "操作失败");
