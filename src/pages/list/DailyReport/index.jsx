...
import { dailyReport, userPackageList, dateReport, gameReport } from "../../../api/index";
...
class DailyReport extends Component {
	...
	onSearchData = async () => {
		...
		let newGameData = this.state.packageList.map(e => {
			return { package_id: parseInt(e.id), win_statement_total: 0, lose_statement_total: 0 };
		});
		if (res?.data?.game) {
		...
		this.setState({ data: newRes.data || [], count: newRes.count, isLoading: false });
	};
	parseData = res => {
		var length = 0;
		var format_data = [];
	...
	parseDateData = res => {
		...
		if (res.status === 0 && res.data) {
			...
			// 排序
			res.data.user.sort(
				(a, b) => parseInt(b.date.replace(/-/g, "")) - parseInt(a.date.replace(/-/g, "")),
			);
		...
		if (res.status === 0 && res.data) {
			...
			success.forEach(data => {
				if (summary[data.game_id]) {
					summary[data.game_id]['win_statement_total'] += data.win_statement_total
					summary[data.game_id]['lose_statement_total'] += data.lose_statement_total
			...
			const newData = Object.entries(summary).map(([game_id, { win_statement_total, lose_statement_total, count, game_name }]) => {
				return {
					game_id,
					win_statement_total,
	...
	getGameReport = async record => {
		...
		const res = await gameReport(1, 20, record.package_id, this.startTime, this.endTime);
		message.destroy("loadingMsg");
		message.info(res.msg);
