				...
				switch (searchData.current.game_id) {
					...
					case "5b1f3a3cb76a591e7f25176": //德州扑克
						reactnode = (
							<React.Fragment>
								{(record?.player_info || []).map((ele, i) => {
									return <div key={i}>{reverseNumber(ele.settlement_funds)}</div>;
								})}
							</React.Fragment>
						);
						break;
		...
		data_for_excel.forEach(ele => {
			...
			columns.forEach(item => {
				...
				if (item.dataIndex === "name") {
					obj[item.title] = new_gameRouter2[searchData.current.game_id].name;
				}
					...
					switch (searchData.current.game_id) {
						case "5b1f3a3cb76a591e7f251711": //斗地主
							res = (record?.settlement || []).map((ele, i) => {
								return `${ele.PlayerId}${
									record?.player_info?.[ele.PlayerId]?.is_robot ? "(robot)" : ""
								}\n`;
							});
							break;
						case "5b1f3a3cb76a591e7f25170": //二人麻将
